...
{!! Form::open(['url' => route('ligne.store'), 'method'=>'POST']) !!}
                 ...
                 <h4>DEVIS OU PROFORMAS</h4>
                  <div class="form-group">
                   <div class="col-sm-12" style="padding-left:0px;">
                     {!! Form::select('produit_type',$produits,null,['class' =>'form-control input-sm','id'=>'produit_list']) !!}
                   </div>
                 </div>
                 <br/>
                 <div class="col-md-12" style="padding-left:0px;" >
                    <h4 class="num-devis"> </h4>
                 </div>
              </div>
          </div>  
          </div>
    ...
    <div class="row">
      ...
      <div class="row">
        <div class="col-md-4">
          <div class=" cadre-info panel-default">
          <div class="panel-body ">
           <p>N° ELB/<span class="num-devis"></span></p>
           <p>DATE DEVIS : {{ $date }}</p>
      ...
      <div class="col-md-5 pull-right">
        <div class=" cadre-info panel-default">
          <div class="panel-body ">
           ...
           <p>N° DEVIS : <span id="num-dv-client"></span></p>
...
@section('script')
  	$(document).ready(function(){
        ...
        $('#msg-offre').hide();
          ...
          <!-- Choix du type de devis par le produit -->
           $("select[name='produit_type']").change(function(){
            var id = $(this).val();
            var data = { "id ": id };
             $.getJSON('ProduitSelect', { id : id }, function(data) {
                console.log(data);
                $('.num-devis').html(data.data.dv_num);
                $('#num-dv-client').html(data.data.num);
          ...
          $("select[name='societe_id']").change(function(){
            ...
            var data = { "id ": id };
             $.getJSON('SocieteSelect', { id : id }, function(data) {
                ...
                $('#nomsociete').html(data.societe.nom_clt);
                $("#adressesociete").html(
                  "<h5>" + data.societe.ville_siege_clt + "</h5><h5> " + data.societe.pays_clt+ "</h5><h5>" + data.societe.adresse_siege_clt + "</h5>"
                );
        ...
        <!-- Choix du contact à lier au devis  -->
          ...
          var template = '<tr id="ligne-article'+n+'" class="supp-ligne">'+
                            '<td>'+
                                '<select name="module_id[]" class="form-control input-sm article_list modulelist'+n+' " id="module_list"></select>'+
